...
const getNodeColor = (d) => {
 ...
 function click(evt, d) {
    ...
    // Chế độ khám phá đồ thị cục bộ
    if (view.mode === 'local_graph') {
        const sourceId = view.fullGraph.getID(clickedNodeId);
        if (sourceId) {
            // Lấy các node hàng xóm từ đồ thị đầy đủ
            for (const edge of view.fullGraph.getOutEdges(sourceId)) {
                const neighborName = view.fullGraph.getName(edge.target);
                // Thêm node và cạnh mới vào đồ thị đang hiển thị
                graph.addEdgeExtended(clickedNodeId, neighborName, 1);
            }
            // Cập nhật lại hiển thị đồ thị
            updateGraph();
        }
        return; // Dừng lại, không mở file
    }

    // Hành vi cũ cho chế độ tìm đường
    if (evt.altKey) {
    ...
    const file = app.vault.getAbstractFileByPath(clickedNodeId);
    if (file instanceof import_obsidian5.TFile) {
      const isMetaKey = evt.metaKey || evt.ctrlKey;
      const leaf = app.workspace.getLeaf(isMetaKey);
      leaf.openFile(file);
    }
  }
...
function drag(simulation2) {
    ...
    function dragended(event, d) {
      ...
      // Nếu phím Alt KHÔNG được nhấn trong khi kéo, hãy bỏ ghim node.
      if (!event.sourceEvent.altKey) {
          delete d.fx;
          delete d.fy;
          select_default2(this).classed("fixed", false);
      } else {
        // Nếu phím Alt được nhấn, đảm bảo node được đánh dấu là đã ghim.
        select_default2(this).classed("fixed", true);
      }
    }
...
var PathFinderPlugin = class extends import_obsidian8.Plugin {
  async onload() {
    ...
    // TÍNH NĂNG MỚI: Thêm lệnh Explore Local Graph
    this.addCommand({
      id: "explore-local-graph",
      name: "Explore Local Graph",
      checkCallback: (checking) => {
        const file = this.app.workspace.getActiveFile();
        if (file) {
          if (!checking) {
            this.findPaths("local_graph", this.settings.filter, file.path);
          }
          return true;
        }
        return false;
...
findPaths(operation, filter2, from, to, length) {
    ...
    // Chỉ xử lý 'to' nếu nó được cung cấp (không phải local_graph)
    if (to) {
    ...
    let target = to ? graph.getID(to) : null; // Lấy target ID nếu 'to' tồn tại
    ...
    // Chỉ kiểm tra target nếu nó cần thiết cho các operation tìm đường
    if (operation !== "local_graph") {
        if (to && target === void 0) {
    ...
    } else { // Xử lý riêng cho local_graph
        this.openPathGraphView(from, null, 1, graph, "local_graph");
    }
  }
  ...
  async openPathGraphView(from, to, length, graph, mode = "path_finding") {
    let { workspace } = app;
    let pathGraphViewLeaf = workspace.getLeaf(true);
   pathGraphViewLeaf.getDisplayText = function() {
        if (mode === 'local_graph') {
            return `Exploring: ${(0, import_posix.basename)(from, (0, import_posix.extname)(from))}`;
        }
    ...
    pathGraphView.setData(from, to, length, graph, mode);
